 ...
 *
 * - Cada RoomParty avisa por `onRequest` (fetch entre parties): upsert / remove.
 ...
 * `remove` desde su `onAlarm` (sin `context.parties`), así que arma la remoción
...
export default class LobbyServer implements Party.Server {
  ...
  async onStart(): Promise<void> {
    const storedRooms = await this.room.storage.get<Record<string, OnlineRoomSummary>>(ROOMS_STORAGE_KEY);
    if (storedRooms) for (const [id, summary] of Object.entries(storedRooms)) this.rooms.set(id, summary);
    const storedPending = await this.room.storage.get<Record<string, number>>(PENDING_STORAGE_KEY);
  ...
  private async persistAndBroadcast(): Promise<void> {
    await this.room.storage.put(ROOMS_STORAGE_KEY, Object.fromEntries(this.rooms));
    this.room.broadcast(this.roomsMessage());
  }

  private async persistPending(): Promise<void> {
    await this.room.storage.put(PENDING_STORAGE_KEY, Object.fromEntries(this.pendingRemoval));
  }
